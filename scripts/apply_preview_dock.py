#!/usr/bin/env python3
"""Apply the shared v8 comparison pill to every static preview route."""
...
DOCK_CSS = r'''
/* Canonical v8 comparison pill: shared across the three preview variants. */
...
.preview-dock *{font:inherit;letter-spacing:normal;text-transform:none}
...
.preview-group-label{flex:0 0 auto;margin:0 5px;color:#bdbdbd}
...
for variant in ("a", "b", "c"):
    ...
    if ".preview-dock{" not in content:
        ...
        content, replaced = re.subn(
            r"/\* Canonical v\d+ comparison pill: shared across the three preview variants\. \*/\n"
            r"(?:[^\n]*\.preview-[^\n]*\n?)+",
            DOCK_CSS.lstrip(), content, count=1,
        )
        if not replaced:
            raise SystemExit(f"missing canonical dock styles in {css}")
        css.write_text(content)
    for page in sorted(variant_root.glob("*.html")):
        content = page.read_text()
        content = re.sub(
            r'href="styles\.css(?:\?v=[^"]*)?"',
            f'href="styles.css?v={ASSET_VERSION}"', content,
        )
        if 'class="preview-dock"' not in content:
            ...
            content = re.sub(
                r'<aside class="preview-dock"[^>]*>.*?</aside>',
                render(variant, page.name), content, count=1, flags=re.DOTALL,
            )
